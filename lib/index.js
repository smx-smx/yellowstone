...
class RtspClient extends EventEmitter {
  constructor(username, password, headers) {
    ...
    this.headers = Object.assign({ "User-Agent": "SVRC/2.2.0" },
        headers || {});
  ...
  _onData(data) {
    ...
    while (index < data.length) {
      ...
      if(this.readState == readStates.READING_FAILED){
        this.emit('data_error', 'RTSP data error');
        return;
      }
      ...
      if (this.readState == readStates.SEARCHING && data[index] == 0x24) { // $
        ...
        if (this.rtspMessage.length >= 2
          ...
          if (this.rtspContentLength == 0) {
            ...
            //NothingYF:
            this.emit("response", this.rtspStatusLine, this.rtspHeaders, [], ascii_text);
            this.readState = readStates.SEARCHING;
          } else {
            this.rtspContent = [];
        ...
        if (this.rtspContent.length == this.rtspContentLength) {
          ...
          this.emit("response", this.rtspStatusLine, this.rtspHeaders, mediaHeaders, String.fromCharCode.apply(null,this.rtspMessage) + ascii_text);
          this.readState = readStates.SEARCHING;
        }
        ...
        // if((n + this.rtspPacketPointer)  > this.rtspPacketLength){
        //   n = this.rtspPacketLength - this.rtspPacketPointer;
        // }
        // this.rtspPacket.copy(data, index, this.rtspPacketPointer, this.rtspPacketPointer + n);
        // index += n;
        // this.rtspPacketPointer += n;
        ...
        if (this.rtspPacketPointer == this.rtspPacketLength) {
          ...
          if (packetChannel === 0 || packetChannel === 2) {
            const packet = parseRTPPacket(this.rtspPacket);
            this.emit("data", packetChannel, packet.payload, packet, this.rtspMessage, this.rtspPacket);
          }
          if (packetChannel === 1 || packetChannel === 3) {
            const packet = parseRTCPPacket(this.rtspPacket, this.rtspMessage, this.rtspPacket);
        ...
        //throw new Error("Bug in RTSP data framing, please file an issue with the author with stacktrace.");
          //this.emit('error', 'Error RTSP data framing');
          this.readState = readStates.READING_FAILED;
      }
    } // end while
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (mediaSource.control) {
        ...
        //this._url += `/${mediaSource.control}`;
          videoUrl = this._url;
          if(mediaSource.control.substr(0, 4).toLowerCase() == 'rtsp'){
              videoUrl = mediaSource.control;
          }else{
              videoUrl += mediaSource.control;
          }
      }
      ...
      return this.request("SETUP", { Transport: "RTP/AVP/TCP;interleaved=0-1" }, videoUrl).then(_=>{
        ...
        //TODO: Audio
          ...
          if (mediaSourceAudio.control) {
              audioUrl = this._url;
              if(mediaSourceAudio.control.substr(0, 4).toLowerCase() == 'rtsp'){
                  audioUrl = mediaSourceAudio.control;
              }else{
                  audioUrl += mediaSourceAudio.control;
              }
          }

          return this.request("SETUP", { Transport: "RTP/AVP/TCP;interleaved=2-3" }, audioUrl);
      });
  ...
  request(requestName, headers, url) {
    ...
    return new Promise((resolve, reject) => {
      const responseHandler = (responseName, headers, mediaHeaders, resp) => {
        ...
        if (statusCode === 200) {
          ...
          this.emit('message', requestName, id, req, resp);
          ...
          if (statusCode === 401) {
            ...
            resolve(this.request(requestName, Object.assign(requestHeaders, { Authorization: authString }), url));
            return;
          }
  ...
  play() {
    ...
    return this.request("PLAY", { Session: this._session }, this._url).then(() => this);
  }
